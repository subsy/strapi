...
const editingLockService = require('../EditingLock');
...
describe('EditingLock', () => {
  ...
  test('updateLastUpdatedAtMetadata', async () => {
    const model = 'application::country.country';
    const entityId = 1;
    const uid = 123;
    const result = {
    ...
    const metadata = { lastActivityDate: new Date() };
    const editMetadata = jest.fn(() => Promise.resolve(result));
    global.strapi.lockService = () => ({ editMetadata });

    const lockResult = await editingLockService.updateLastUpdatedAtMetadata({
      model,
      entityId,
      metadata,
    });
